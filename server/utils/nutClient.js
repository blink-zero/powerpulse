...
// Add the connectToNutServer function for the tests
async function connectToNutServer(host, port, username, password) {
  try {
    const client = await getNutClient(host, port, username, password);
...
// Add the closeNutConnection function for the tests
async function closeNutConnection(client) {
  return new Promise((resolve, reject) => {
    client.close((error) => {
      if (error) {
        reject(new Error(`Failed to close connection: ${error}`));
        return;
      }
